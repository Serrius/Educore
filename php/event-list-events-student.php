...
try {
    ...
    $dept = strtoupper(trim((string)$dept)); // can be ''
    ...
    // Rule:
    //   Show ORG events if:
    //     - org is GENERAL (o.scope='general')  -> visible to everyone
    //       OR
    //     - org.course_abbr matches user's department (department orgs)
    //
    // Notes:
    //   - We alias created_at -> event_date (since event_date column doesn't exist)
    //   - We include general org even if user has no dept
    $sql = "
        SELECT
            e.id,
            e.title,
        ...
        WHERE
            ...
            AND (
                LOWER(o.scope) = 'general'
                " . ($dept !== '' ? " OR UPPER(o.course_abbr) = :dept " : "") . "
            )
    ";
    ...
    jerr(500, 'Failed to load events', ['error' => $e->getMessage()]);
}
